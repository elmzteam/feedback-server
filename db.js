...
var db = mongo.connect(dbPath, ["users", "restaurants", "items", "sessions", "queries", "prefs", "brains"]);
...
/**
 * Returns the result of a database query if there is exactly one match, and otherwise rejects.
 * @param {string} collection The collection on which to run the query
 * @param {object} query The query to run on the collection (formatted mongo-style)
 * @param {string} reject The argument with which to reject if multiple documents are returned
 * @returns {Promise}
 */
